...
void VineSystem::update(float dt, sf::Vector2i mousePos, float width, float height) {
    m_temps += dt;
    m_globalAgitation *= std::pow(0.01f, dt); // amortissement rapide

    for (auto& l : m_lianes) {
        if (l.currentLength < l.targetLength)
            l.currentLength += 80.f * dt;

        if (mousePos.y > 0 && mousePos.y < l.currentLength + 30.f) {
            float basePhase = m_temps * l.swaySpeed + l.phase;
            float currentX  = l.startX + std::sin(basePhase) * (mousePos.y * 0.02f);
            if (std::abs(mousePos.x - currentX) < 20.f * l.scaleMultiplier)
                l.agitation = std::min(l.agitation + 4.f * dt, 0.6f);
        }
        // La globalAgitation booste l'agitation individuelle
        l.agitation = std::min(l.agitation + m_globalAgitation * dt, 2.0f);
        l.agitation = std::max(0.f, l.agitation - 1.5f * dt);
    }
}
